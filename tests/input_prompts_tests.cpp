...
void RunInputPromptTests() {
    ...
    PBVP_CHECK(pbvp::FormatCatalogPrompt(labels, output));
    PBVP_CHECK(Text(output) == "ENTER PLAY  UP/DOWN SELECT");

    PBVP_CHECK(pbvp::FormatCatalogBackPrompt(labels, output));
    PBVP_CHECK(Text(output) == "ESC BACK");

    pbvp::PlaybackStateSnapshot playback{};
    playback.state = pbvp::PlaybackState::playing;
    PBVP_CHECK(pbvp::FormatPlaybackPrompt(playback, labels, output));
    ...
    PBVP_CHECK(pbvp::FormatPlaybackPrompt(playback, labels, output));
    PBVP_CHECK(Text(output) ==
               "PAUSED  SPACE RESUME  ESC STOP  LEFT/RIGHT SEEK  T COLOR");

    playback.state = pbvp::PlaybackState::buffering;
    playback.pause_after_buffering = false;
    PBVP_CHECK(pbvp::FormatPlaybackPrompt(playback, labels, output));
    PBVP_CHECK(Text(output) == "BUFFERING  ESC STOP");
    ...
    PBVP_CHECK(pbvp::FormatPlaybackPrompt(playback, {}, output));
    ...
    PBVP_CHECK(!pbvp::FormatCatalogPrompt(labels, short_output));
    PBVP_CHECK(short_output.back() == '\0');
    PBVP_CHECK(!pbvp::FormatCatalogPrompt({}, output));
    PBVP_CHECK(!pbvp::FormatCatalogBackPrompt({}, output));
    PBVP_CHECK(!pbvp::FormatPlaybackPrompt(
        {pbvp::PlaybackState::playing, pbvp::PlaybackError::none, false},
        {}, output));
    PBVP_CHECK(!pbvp::FormatPlaybackPrompt({}, {}, std::span<char>{}));
}
