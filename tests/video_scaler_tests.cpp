...
void RunVideoScalerTests() {
...

    {
        std::vector<std::uint8_t> wide(16u * 9u * 4u, 0x7Fu);
        std::vector<std::uint8_t> square(16u * 16u * 4u, 0xCCu);
        const auto result = pbvp::ScaleBgraForPresentation(
            wide, 16u, 9u, 64u, square, 16u, 16u, 64u,
            {16.0f, 9.0f}, pbvp::VideoScaleMode::fit,
            pbvp::VideoColorMode::full_color);
        PBVP_CHECK(result.status == pbvp::VideoScaleStatus::ok);
        PBVP_CHECK(result.content_x == 0u);
        PBVP_CHECK(result.content_y == 0u);
        PBVP_CHECK(result.content_width == 16u);
        PBVP_CHECK(result.content_height == 16u);
        PBVP_CHECK(Pixel(square, 64u, 8u, 0u) == 0xFF7F7F7Fu);
        PBVP_CHECK(Pixel(square, 64u, 8u, 15u) == 0xFF7F7F7Fu);
    }

    {
        std::vector<std::uint8_t> standard(12u * 9u * 4u, 0x55u);
        std::vector<std::uint8_t> square(16u * 16u * 4u, 0xCCu);
        const auto result = pbvp::ScaleBgraForPresentation(
            standard, 12u, 9u, 48u, square, 16u, 16u, 64u,
            {16.0f, 9.0f}, pbvp::VideoScaleMode::fit,
            pbvp::VideoColorMode::full_color);
        PBVP_CHECK(result.status == pbvp::VideoScaleStatus::ok);
        PBVP_CHECK(result.content_x == 2u);
        PBVP_CHECK(result.content_y == 0u);
        PBVP_CHECK(result.content_width == 12u);
        PBVP_CHECK(result.content_height == 16u);
        PBVP_CHECK(Pixel(square, 64u, 1u, 8u) == 0xFF000000u);
        PBVP_CHECK(Pixel(square, 64u, 2u, 8u) == 0xFF555555u);
        PBVP_CHECK(Pixel(square, 64u, 14u, 8u) == 0xFF000000u);
    }

    {
        std::vector<std::uint8_t> standard(12u * 9u * 4u, 0u);
        for (std::uint32_t y = 0u; y < 9u; ++y) {
            for (std::uint32_t x = 0u; x < 12u; ++x) {
                const std::size_t offset = static_cast<std::size_t>(y * 12u + x) * 4u;
                standard[offset] = static_cast<std::uint8_t>(y);
                standard[offset + 3u] = 255u;
            }
        }
        std::vector<std::uint8_t> square(16u * 16u * 4u, 0xCCu);
        const auto result = pbvp::ScaleBgraForPresentation(
            standard, 12u, 9u, 48u, square, 16u, 16u, 64u,
            {16.0f, 9.0f}, pbvp::VideoScaleMode::fill,
            pbvp::VideoColorMode::full_color);
        PBVP_CHECK(result.status == pbvp::VideoScaleStatus::ok);
        PBVP_CHECK(result.content_x == 0u);
        PBVP_CHECK(result.content_y == 0u);
        PBVP_CHECK(result.content_width == 16u);
        PBVP_CHECK(result.content_height == 16u);
        PBVP_CHECK(Pixel(square, 64u, 8u, 0u) == 0xFF000001u);
        PBVP_CHECK(Pixel(square, 64u, 8u, 15u) == 0xFF000006u);
    }
...

    {
        ...
        PBVP_CHECK(pbvp::ScaleBgraForPresentation(
            bytes, 1u, 1u, 4u, bytes, 1u, 1u, 4u,
            {0.0f, 1.0f}, pbvp::VideoScaleMode::fit,
            pbvp::VideoColorMode::full_color).status ==
                   pbvp::VideoScaleStatus::invalid_presentation);
    }
}
