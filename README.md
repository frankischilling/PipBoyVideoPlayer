...
The host and Win32 automated tests pass. Two overlay draw points were rejected because they rendered above the Pip-Boy UI. The accepted path updates an engine-owned managed texture and leaves drawing to the game. It uses no executable hook or Direct3D device vtable patch. The raised 384x216 panel passed visual checks in the Base and full Extended UI profiles at 1920x1080. Keyboard and mouse input, ten Pip-Boy reopen cycles, and 50 measured focus-loss and return cycles passed in the tested native windowed configuration. The latest Extended run uploaded twice in 24.30 to 28.70 microseconds, recorded no upload failure, and shut down cleanly.

A synthetic recreation test froze inside the game's native reset sequence, so the test, reset hook, and MinHook dependency were removed. A PBVP-disabled control later reproduced the native fullscreen NVIDIA driver crash, so repeated fullscreen Alt+Tab is not a supported claim. The isolated test-profile save guard now passes both Base and full Extended exit checks without changing normal profiles. Controller input, remaining resolutions and UI profiles, FPS caps, and DXVK support are not verified yet.
