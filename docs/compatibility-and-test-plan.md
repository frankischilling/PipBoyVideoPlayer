...
The short diagnostic recorded 32 controller updates before the fourth underrun. At failure, the audio clock was 2,816,000 microseconds, the decoder had supplied 86 video frames, and the controller had submitted 140,288 audio samples. XAudio2 had played 135,168 samples and still had five buffers queued after the final refill. The decoder retained two video items and two audio items, so neither decoder queue was full. The preserved log has SHA-256 `7E8721DF3B317093411CBEDAA780FA3C9ACAE4E1CFD5A9E07C0BABBBC409B2D5`.

Code inspection found the missing state transition. `BeginRebuffer` changes the playback state, but the started XAudio2 voice keeps running. Samples submitted during `buffering` are consumed while the controller is trying to rebuild its 200 millisecond threshold. The next candidate pauses the source voice during recovery and resumes it only after the fixed threshold is ready. The five-minute row remains open until that build passes without an underrun.
...
The service-order candidate failed a valid live run with no Alt+Tab. Playback first changed from `playing` to `buffering` after 2.6 seconds. Recovery took one 100 millisecond update, but later underruns occurred after 500 and 400 milliseconds. The fourth underrun produced `audio_stream_failed` 4.1 seconds after playback began. Visible cadence stayed between 9.91 and 10.08 FPS.

At failure, the controller had run 44 updates, decoded 113 frames, submitted 183,296 samples, and played 178,176 samples. XAudio2 held five buffers, while the last recorded decoder usage held two video items and two audio items. The preserved log has SHA-256 `4546A674D91B3CFD7F1E2F12F64D2CA861A90BD59DCC5F4FBDA5F949A919F813`.

Code inspection found a separate recovery defect in `XAudioStream::ReadSnapshot`. The function calls `ReadyToStart` before `ReclaimCompleted`, so finished slots can still count toward the 200 millisecond threshold. The same snapshot can report that recovery is ready and then report only five live XAudio2 buffers after reclamation. The next candidate reclaims completed slots before calculating readiness and adds a native regression for this ordering. The real long fixture must then run at a 100 millisecond controller cadence before the initial prebuffer changes. The first live underrun remains unresolved until that regression provides a controlled measurement.

The native 100 millisecond cadence test reproduced the live starvation with the existing three-item video queue and 200 millisecond audio prebuffer. After 55 updates, it had decoded 159 frames, delivered 51, dropped 107, submitted 257,024 audio samples, reached a 5,248,000 microsecond clock, emptied the XAudio2 queue, and recorded one underrun. Changing only the audio prebuffer to 300 milliseconds still emptied the queue and recorded one underrun.

A six-item video queue completed six-second checks with both 300 and 200 millisecond prebuffers, zero underruns, and four or five XAudio2 buffers remaining. The six-item queue and 200 millisecond prebuffer then completed a 30-second controlled run in `playing` with zero underruns. It decoded 897 frames, delivered 273, dropped 623 late frames, submitted 1,440,768 samples, reached a 29,830,000 microsecond clock, and retained four XAudio2 buffers. The temporary update-count minimum was corrected from 280 to 270 because startup buffering is outside steady playback. The production candidate raises only the video item limit to six. The 32 MiB video byte cap, 200 millisecond audio prebuffer, and 16-slot audio pool remain unchanged. The live five-minute gate remains open.
