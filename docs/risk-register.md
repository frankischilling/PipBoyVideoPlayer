...
| UI rectangle is not portable across Pip-Boy layouts | High | High | Validate both UIO registration lines, inspect UIO's add or inject result, keep coordinates in XML, and limit the supported replacer list | The first registration omitted its condition line, so UIO checked the prefab without injecting it | Phases 1 and 5 |
