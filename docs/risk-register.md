...
| Audio underruns on slower systems | High until service-gap tolerance passes live | High | Measure the maximum owner-thread update gap, reproduce it with real XAudio2, then select bounded prebuffer and pool depth from that evidence | The 12-item queues pass five native minutes but the live run still failed at a 126.656 s clock | Phases 3 and 4 |
