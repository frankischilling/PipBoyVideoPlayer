...
| UI rectangle is not portable across Pip-Boy layouts | High | High | Keep the accepted 384 by 216 locus-owned panel and verify the raised shared position in Extended, then test every required layout, resolution, and scale | The raised position clears the Base map buttons; the Extended regression is still open | Phases 1 and 5 |
