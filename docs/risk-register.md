...
| Direct3D device reset conflicts with another plugin | Medium | High | The Extended profile accepted the verified hook, recreation return handling fails closed, and an isolated live conflict fixture passes. Keep the normal build installed while a private diagnostic identifies the requested-size precondition, and do not accept the deferred byte as reset evidence until the game consumes it and enters the reviewed helper | The first August 10 request did not reach the recreation hook. The clean follow-up refused to write because a renderer or requested-size precondition was unavailable. A failed recreation, unknown return value, missing replacement device, black texture, or crash also triggers review | Phase 1 |
