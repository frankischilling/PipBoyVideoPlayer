...
| Video payload allocation fails on the loaded game heap | Low after the short live retest, pending the 30-minute run | High | Keep 512-pixel decoded output and existing queue limits, but allocate only video payloads through checked `VirtualAlloc` regions | The isolated allocator displayed about 56 seconds of video with 1,659 successful uploads and no allocation error after both C++ heap sizes had failed | Phase 4 |
| The long-playback checker rejects a valid early clock offset | Low after the measured live result | Medium | Apply the 50 ms duration tolerance on both sides and retain the separate 50 ms audio-to-video difference check | The first complete run had 28.125 ms absolute error but failed a one-sided exact-duration lower bound | Phase 4 |
