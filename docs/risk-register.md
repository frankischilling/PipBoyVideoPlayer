...
| Repeated focus changes trigger a graphics-driver crash | Medium | High | Run matched 50-cycle tests with the PBVP development mod disabled and enabled before changing the renderer | The first measured PBVP run crashed after four cycles inside `nvd3dum`; a pre-PBVP crash has the same driver version and final five driver frames | Phase 1 |
