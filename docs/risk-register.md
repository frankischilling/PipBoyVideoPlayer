...
| Direct3D device reset conflicts with another plugin | Low to medium | High | The Extended profile accepted the verified hook, recreation return handling now fails closed, and five Alt+Tab switches preserved the draw without invoking recreation; next force an actual display recreation | Failed recreation, an unknown return value, a missing replacement device, a black texture after a display transition, or a crash during another hook's reset work | Phase 1 |
