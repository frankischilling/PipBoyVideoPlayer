...
| --- | --- | --- | --- | --- | --- |
| No safe Pip-Boy render point exists | Medium to high | Project-blocking | The frame-present path failed layer order; the next build uses a checked normal-frame call before the engine UI routine and keeps an engine-owned `TileImage` texture as the fallback | The Extended fullscreen 1920x1080 layer probe showed the checkerboard but neither overlapping UIO element | Phase 1 |
