...
| UI rectangle is not portable across Pip-Boy layouts | High | High | UIO injection and named-tile lookup are verified on Extended; resolve the logical canvas from bounded menu ancestors, keep coordinates in XML, and limit the supported replacer list | `MapMenu` has no screen extent, so the bridge must use the shared logical canvas above it | Phases 1 and 5 |
