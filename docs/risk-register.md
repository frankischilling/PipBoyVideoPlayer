...
| Audio underruns on slower systems | High at a measured 10 FPS until the recovery retest | High | Drain excess bounded video during every buffering state so the interleaved worker can rebuild the fixed audio prebuffer | The first 10 FPS live run returned to buffering after 499 ms and never recovered before shutdown | Phases 3 and 4 |
