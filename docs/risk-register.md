...
| Direct3D device reset conflicts with another plugin | Medium | High | The Extended profile accepted the verified hook, recreation return handling fails closed, and an isolated live conflict fixture passes. Do not accept the deferred request byte as reset evidence until a diagnostic proves that the game consumes it and enters the reviewed helper | The August 10 request was logged, but no recreation start or result followed. A failed recreation, unknown return value, missing replacement device, black texture, or crash also triggers review | Phase 1 |
