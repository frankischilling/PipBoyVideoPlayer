...
| Isolated compatibility runs create game saves | Medium until the corrected guard passes Extended | High | Keep MO2 local saves enabled, require section-aware Phase 1 overrides, refuse profiles that already contain saves, and quarantine unexpected test output | Base stayed empty, but full Extended created a save pair because two guard keys were under the wrong INI section | Phases 1 and 6 |
...
| UI rectangle is not portable across Pip-Boy layouts | Medium | High | Keep the accepted raised 384 by 216 locus-owned panel, then test every remaining required layout, resolution, and scale | The raised position clears the Base map buttons and passed the full Extended regression | Phases 1 and 5 |
