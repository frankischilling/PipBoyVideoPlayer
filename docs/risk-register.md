...
| FFmpeg consumes too much 32-bit address space | Medium in integrated VNV playback | High | Capture the allocation site and free address-space layout before changing queue or output dimensions; keep the current limits until the measurement identifies the failing owner | The short 1080p live decode stayed below budget, but the corrected long-playback build raised `allocation_failed` before its first presented frame | Phase 4 |
