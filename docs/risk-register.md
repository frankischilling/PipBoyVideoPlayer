...
| Direct3D device reset conflicts with another plugin | Medium | High | The Extended profile accepted the verified hook, recreation return handling fails closed, and an isolated live conflict fixture passes. The next private test may fill the zero transient requested-size values only when the active render size and backbuffer agree, then must restore the original values after the deferred request completes | Both requested-size values were readable and zero while the renderer and 1920x1080 backbuffer were valid. A mismatch, failed restoration, failed recreation, unknown return value, missing replacement device, black texture, or crash triggers review | Phase 1 |
