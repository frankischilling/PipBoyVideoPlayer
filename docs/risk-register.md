...
| XAudio2 callback reaches released memory | Low with the planned ownership order | High | Use stable preallocated slots, atomics only in callbacks, destroy the source voice before callback targets, and test repeated shutdown | A callback or audio read occurs after its target is released | Phase 3 |
