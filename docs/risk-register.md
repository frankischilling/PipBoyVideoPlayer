...
| UI rectangle is not portable across Pip-Boy layouts | High | High | Resolve whether playback is a full-glass stage or a smaller lower-left panel, then test every required layout, resolution, and scale before narrowing the support list | The lower-left expression moved the live 560 by 315 tile by the expected amount, but the user still judged its visual footprint to be top-left | Phases 1 and 5 |
