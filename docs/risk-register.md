...
| UIO surface is covered by MapMenu page content | Medium | High | Give each PBVP drawable an explicit depth from 10 through 12, above active page content through depth 8 and below headline and tab controls at depths 15 and 22 | Raising only `PBVP_Root` did not change the transient Items-to-Data result; the active UI assigns depth directly to drawable children | Phase 1 |
