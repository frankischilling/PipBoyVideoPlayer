...
| Direct3D device reset conflicts with the active renderer stack | High | High | Retire the synthetic request and restore the normal build. Reassess whether an observation detour is justified when PBVP retains no default-pool resource | A guarded 1920x1080 same-size request entered `NiDX9Renderer::Recreate` and froze before the native call returned. No crash dump or orderly shutdown record was produced | Phase 1 |
