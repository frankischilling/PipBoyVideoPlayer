...
| Isolated compatibility runs create game saves | Low after guard | High | Keep MO2 local saves enabled, require the Phase 1 multi-INI guard, refuse profiles that already contain saves, and quarantine unexpected test output | The first windowed Extended run created a new save and co-save because its source INI enabled save-on-exit | Phases 1 and 6 |
...
| UI rectangle is not portable across Pip-Boy layouts | High | High | Keep the accepted 384 by 216 locus-owned panel, raise the shared candidate by 52 logical units, and verify every required layout, resolution, and scale | The Extended position passed, but the first Base VNV run showed that it covered the Local Map and World Map buttons | Phases 1 and 5 |
