...
| UIO image has no live engine texture | High | Project-blocking | The static filename produced a valid `TileImage` with a null texture reference; request the verified private filename once through `Tile::SetStringValue` on the game thread and compare a known image field before considering lower-level ownership changes | `PBVP UI LAYER` rendered, the callbacks shared a thread, and the packaged DDS passed the 32-bit DirectX image-info check, but `TileImage + 0x3C` stayed null | Phase 1 |
