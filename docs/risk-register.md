...
| UI rectangle is not portable across Pip-Boy layouts | High | High | Test the requested 320 by 180 lower-left diagnostic panel, then verify every required layout, resolution, and scale before treating it as a release viewport | The lower-left expression moved the live 560 by 315 tile by the expected amount, but that footprint still covered the upper radio station list | Phases 1 and 5 |
