...
| UI rectangle is not portable across Pip-Boy layouts | High | High | UIO injection is now verified on Extended; diagnose the native tile lookup, keep coordinates in XML, and limit the supported replacer list | UIO injected the corrected prefab, but the second run still produced no resolved rectangle or draw | Phases 1 and 5 |
