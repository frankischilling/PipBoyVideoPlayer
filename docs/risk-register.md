...
| Video payload allocation fails on the loaded game heap | High until the allocator retest | High | Keep 512-pixel decoded output and existing queue limits, but allocate only video payloads through checked `VirtualAlloc` regions | Source-sized and 589,824-byte C++ allocations both failed in live playback while more than 1.5 GB remained free in one contiguous region | Phase 4 |
