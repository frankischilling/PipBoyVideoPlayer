...
The live bridge resolved that candidate at `42,276` through `602,591`, compared with `110,108` through `670,423` before the change. The relative anchor therefore worked, but the user still described the checkerboard as top-left. The remaining issue is visual footprint rather than trait propagation. At 560 by 315, a bottom-anchored surface still reaches high into the content area. A smaller lower-left panel would depart from the current full-glass playback contract and needs an explicit design decision before implementation.
