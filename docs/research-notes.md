...
The next diagnostic must separate three facts: whether the request byte changes back to zero, whether the main-loop helper at `0x004DC360` is entered, and whether its direct call reaches the reviewed renderer owner. No direct reset call is justified by the current result.
